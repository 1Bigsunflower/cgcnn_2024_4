...
class StruData(Dataset):
    ...
    def __init__(self, input, output=None, max_num_nbr=12, radius=8, dmin=0, step=0.2, ):
        ...
        atom_init_file = os.path.join('atom.json')
        assert os.path.exists(atom_init_file), 'atom.json does not exist!'
    ...
    def __getitem__(self, idx):
        ...
        # 获取每个原子的特征向量
        atom_fea = np.hstack([self.ari.get_atom_fea(crystal[i].specie.number)
